// white-box-testing.cpp : 定义控制台应用程序的入口点。
//
...
FILE *fp_test_result = fopen("output test result.txt", "w");
int blank_sum_before;        //空格数
...
//输出空格
void fprint_blank(int blank_sum)
{
	int i;
	for (i = 0; i < blank_sum; i++)
	{
		fprintf(fp_test_result, " ");
	}
}


//输出备份距离送达的时间
void fprint_backup_arrive_time_remaining(int component_num, int backup_place)
{
	int i;
	int len_temp, node_temp;
	len_temp = component[component_num].backup_arrive_time_remaining[backup_place].size();
	for (i = 0; i < len_temp; i++)
	{
		node_temp = component[component_num].backup_arrive_time_remaining[backup_place].front();
		component[component_num].backup_arrive_time_remaining[backup_place].pop();
		fprintf(fp_test_result, "%d ", node_temp);
		component[component_num].backup_arrive_time_remaining[backup_place].push(node_temp);
	}
}


//输出损坏的零件距离送达修复地点的时间
void fprint_repair_arrive_time_remaining(int component_num)
{
	int i;
	int len_temp, node_temp;
	len_temp = component[component_num].repair_arrive_time_remaining.size();
	for (i = 0; i < len_temp; i++)
	{
		node_temp = component[component_num].repair_arrive_time_remaining.front();
		component[component_num].repair_arrive_time_remaining.pop();
		fprintf(fp_test_result, "%d ", node_temp);
		component[component_num].repair_arrive_time_remaining.push(node_temp);
	}
}


//输出损坏的零件距离送达修复好的时间
void fprint_repair_recover_time_remaining(int component_num)
{
	int i;
	int len_temp, node_temp;
	len_temp = component[component_num].repair_recover_time_remaining.size();
	for (i = 0; i < len_temp; i++)
	{
		node_temp = component[component_num].repair_recover_time_remaining.front();
		component[component_num].repair_recover_time_remaining.pop();
		fprintf(fp_test_result, "%d ", node_temp);
		component[component_num].repair_recover_time_remaining.push(node_temp);
	}
}


//输出等待修复的零件
void fprint_component_waiting_for_repair(int repair_place)
{
	int i;
	int len_temp, node_temp;
	len_temp = component_waiting_for_repair[repair_place].size();
	for (i = 0; i < len_temp; i++)
	{
		node_temp = component_waiting_for_repair[repair_place].front();
		component_waiting_for_repair[repair_place].pop();
		fprintf(fp_test_result, "%d ", node_temp);
		component_waiting_for_repair[repair_place].push(node_temp);
	}
}
...
void repair_component(int component_num)
{
	...
	fprint_blank(blank_sum_before);
	fprintf(fp_test_result, "repair component %d\n", component_num);
	fprint_blank(blank_sum_before + 4);
	fprintf(fp_test_result, "resource needed: ");
	p = component[component_num].resource;
	while (p != NULL)
	{
		fprintf(fp_test_result, "%d(%d) ", p->node_num, p->node_sum);
		p = p->next;
	}
	fprintf(fp_test_result, "\n");
	fprint_blank(blank_sum_before + 4);
	fprintf(fp_test_result, "resource available: ");
	p = component[component_num].resource;
	while (p != NULL)
	{
		fprintf(fp_test_result, "%d(%d) ", p->node_num, resource[p->node_num].backup_available_sum[component[component_num].repair_place]);
		p = p->next;
	}
	fprintf(fp_test_result, "\n");
	...
	if (flag == 1)
	{
		...
		fprint_blank(blank_sum_before + 4);
		fprintf(fp_test_result, "resource enough\n");
		fprint_blank(blank_sum_before + 4);
		fprintf(fp_test_result, "resource available: ");
		p = component[component_num].resource;
		while (p != NULL)
		...
		fprintf(fp_test_result, "\n");
		fprint_blank(blank_sum_before + 4);
		fprintf(fp_test_result, "repair recover time remaining: ");
		fprint_repair_recover_time_remaining(component_num);
		fprintf(fp_test_result, "\n");
	}
	...
	else
	{
		...
		fprint_blank(blank_sum_before + 4);
		fprintf(fp_test_result, "resource not enough\n");
		fprint_blank(blank_sum_before + 4);
		fprintf(fp_test_result, "component waiting for repair: ");
		fprint_component_waiting_for_repair(component[component_num].repair_place);
		fprintf(fp_test_result, "\n");
	}
}
...
void change_machine(int machine_num, int time_now)
{
	...
	fprint_blank(blank_sum_before);
	fprintf(fp_test_result, "change machine %d\n", machine_num);
	fprint_blank(blank_sum_before + 4);
	fprintf(fp_test_result, "component needed: ");
	...
	fprintf(fp_test_result, "\n");
	fprint_blank(blank_sum_before + 4);
	fprintf(fp_test_result, "component available: ");
	p = machine[machine_num].component;
	while (p != NULL)
	{
		fprintf(fp_test_result, "%d(%d) ", p->node_num, component[p->node_num].backup_available_sum[0]);
		p = p->next;
	}
	fprintf(fp_test_result, "\n");
	...
	if (flag == 1)
	{
		...
		fprint_blank(blank_sum_before + 4);
		fprintf(fp_test_result, "component enough\n");
		fprint_blank(blank_sum_before + 4);
		fprintf(fp_test_result, "component available: ");
		p = machine[machine_num].component;
		while (p != NULL)
		...
		fprintf(fp_test_result, "\n");
		fprint_blank(blank_sum_before + 4);
		fprintf(fp_test_result, "reliability move step: %d\n", machine[machine_num].reliability_move_step);
		fprint_blank(blank_sum_before + 4);
		fprintf(fp_test_result, "capacity: %lf\n", machine[machine_num].capacity);
		fprint_blank(blank_sum_before + 4);
		fprintf(fp_test_result, "backup recover time remaining: %d\n", machine[machine_num].backup_recover_time_remaining);
	}
	...
	{
		fprint_blank(blank_sum_before + 4);
		fprintf(fp_test_result, "component not enough\n");
	}
}
...
void repair_machine(int machine_num)
{
	...
	fprint_blank(blank_sum_before);
	fprintf(fp_test_result, "repair machine %d\n", machine_num);
	fprint_blank(blank_sum_before + 4);
	fprintf(fp_test_result, "component to repair: ");
	p = machine[machine_num].component;
	while (p != NULL)
	{
		fprintf(fp_test_result, "%d(%d) ", p->node_num, p->node_sum);
	...
	while (p != NULL)
	{
		...
		else
		{
			for (i = 0; i < p->node_sum; i++)
			{
				...
				fprint_blank(blank_sum_before + 4);
				fprintf(fp_test_result, "transport component %d to repair place %d\n", p->node_num, component[p->node_num].repair_place);
				fprint_blank(blank_sum_before + 8);
				fprintf(fp_test_result, "repair arrive time remaining: ");
				fprint_repair_arrive_time_remaining(p->node_num);
				fprintf(fp_test_result, "\n");
			}
...
//		printf("  time: %d\n", i);
		...
		if (i%preventive_repair_time == 0)
		{
			...
			{
				fprint_blank(8);
				fprintf(fp_test_result, "examine machine %d\n", j);
				fprint_blank(12);
				fprintf(fp_test_result, "repair reliability: %lf\n", machine[j].repair_reliability);
				fprint_blank(12);
				fprintf(fp_test_result, "reliability: %lf\n", machine[j].reliability[i - machine[j].reliability_move_step]);
		...
		{
			fprint_blank(8);
			fprintf(fp_test_result, "examine function %d\n", j);
			...
			while (p != NULL&&flag == 1)
			{
				...
				if (machine[p->node_num].fault == 0)
				{
					...
					fprint_blank(16);
					fprintf(fp_test_result, "probability: %.9lf\n", probability);
					fprint_blank(16);
					fprintf(fp_test_result, "reliability: %lf\n", machine[p->node_num].reliability[i - machine[p->node_num].reliability_move_step]);
					blank_sum_before = 16;
					if (probability > machine[p->node_num].reliability[i - machine[p->node_num].reliability_move_step])
					{
						...
						fprintf(fp_test_result, "probability: %.9lf\n", probability);
						fprint_blank(16);
						fprintf(fp_test_result, "unrecoverable rate: %.9lf\n", machine[p->node_num].repair_unrecoverable_rate);
				...
				fprint_blank(16);
				fprintf(fp_test_result, "capacity: %lf\n", machine[p->node_num].capacity);
				fprint_blank(16);
				fprintf(fp_test_result, "used: %d\n", machine[p->node_num].used);
				fprint_blank(12);
				fprintf(fp_test_result, "capacity: %lf\n", function[j].capacity);
		...
		for (j = 0; j < N_function; j++)
		{
			...
			fprintf(fp_test_result, "examine function %d\n", j);
			fprint_blank(12);
			fprintf(fp_test_result, "fault: %d\n", node[j + N_process].fault);
		}
		for (j = 0; j < N_process_exist; j++)
		{
			fprint_blank(8);
			fprintf(fp_test_result, "examine process %d\n", j);
			fprint_blank(12);
			fprintf(fp_test_result, "fault: %d\n", node[j].fault);
		}
		...
		if (node[sequence_process[N_process_exist]].fault == 0)
		{
			...
			for (j = 0; j < N_function; j++)
			{
				...
				fprintf(fp_test_result, "capacity: %lf\n", function[j].capacity);
				fprint_blank(8);
				fprintf(fp_test_result, "system capacity: %lf\n", system_capacity);
			}
			...
			for (j = 0; j < N_machine; j++)
			{
				...
				fprint_blank(8);
				fprintf(fp_test_result, "refresh machine %d\n", j);
				fprint_blank(12);
				fprintf(fp_test_result, "used: %d\n", machine[j].used);
				fprint_blank(12);
				fprintf(fp_test_result, "backup recover time remaining: %d\n", machine[j].backup_recover_time_remaining);
		...
		{
			fprint_blank(8);
			fprintf(fp_test_result, "refresh component %d\n", j);

			//遍历基层和中继
			for (k = 0; k < backup_place_sum - 1; k++)
			{
				fprint_blank(12);
				fprintf(fp_test_result, "refresh backup place %d\n", k);

				fprint_blank(16);
				fprintf(fp_test_result, "backup available sum: %d\n", component[j].backup_available_sum[k]);
				fprint_blank(16);
				fprintf(fp_test_result, "backup minimum sum: %d\n", component[j].backup_minimum_sum[k]);
				...
				if (component[j].backup_available_sum[k] < component[j].backup_minimum_sum[k])
				{
					...
					fprint_blank(16);
					fprintf(fp_test_result, "transport %d component %d to backup place %d\n", len_temp, j, k);
					fprint_blank(20);
					fprintf(fp_test_result, "backup arrive time remaining: ");
					fprint_backup_arrive_time_remaining(j, k);
					fprintf(fp_test_result, "\n");
				}
			...
			for (k = 0; k < len_temp; k++)
			{
				...
				{
					fprint_blank(12);
					fprintf(fp_test_result, "repair finished\n");
					...
					while (p != NULL)
					{
						fprintf(fp_test_result, "%d(%d) ", p->node_num, resource[p->node_num].backup_available_sum[component[j].repair_place]);
						p = p->next;
					}
					fprintf(fp_test_result, "\n");
					fprint_blank(16);
					fprintf(fp_test_result, "resource to release: ");
					p = component[j].resource;
					while (p != NULL)
					{
						fprintf(fp_test_result, "%d(%d) ", p->node_num, p->node_sum);
				...
				else
				{
					...
					fprint_blank(12);
					fprintf(fp_test_result, "repair unfinished\n");
					fprint_blank(16);
					fprintf(fp_test_result, "repair recover time remaining: ");
					fprint_repair_recover_time_remaining(j);
					fprintf(fp_test_result, "\n");
				}
			}
			...
			fprint_blank(12);
			fprintf(fp_test_result, "repair arrive time remaining: ");
			fprint_repair_arrive_time_remaining(j);
			fprintf(fp_test_result, "\n");
			fprint_blank(12);
			fprintf(fp_test_result, "component waiting for repair: ");
			fprint_component_waiting_for_repair(component[j].repair_place);
			fprintf(fp_test_result, "\n");
		}
		...
		{
			fprint_blank(8);
			fprintf(fp_test_result, "refresh component waiting for repair %d\n", j);
...
void fprint_adjacency_list()
{
	...
	Linked_List *p;
	FILE *fp1, *fp2;
	fp1 = fopen("output adjacency list forward.txt", "w");
	fp2 = fopen("output adjacency list reverse.txt", "w");
	for (i = 0; i < N_process + N_function; i++)
	{
		p = adjacency_list_forward[i];
		fprintf(fp1, "%d (%d): ", i, node[i].in_degree);
		while (p != NULL)
		{
			fprintf(fp1, "%d ", p->node_num);
			p = p->next;
		}
		fprintf(fp1, "\n");

		p = adjacency_list_reverse[i];
		fprintf(fp2, "%d (%d): ", i, node[i].out_degree);
		while (p != NULL)
		{
			fprintf(fp2, "%d ", p->node_num);
			p = p->next;
		}
		fprintf(fp2, "\n");
	}
...
//输出邻接矩阵
void fprint_adjacency_matrix()
{
	int i, j;
	FILE *fp = fopen("output adjacency matrix forward.txt", "w");
	for (i = 0; i < N_process + N_function; i++)
	{
		for (j = 0; j < N_process + N_function; j++)
		{
			fprintf(fp, "%2d", adjacency_matrix_forward[i][j]);
		}
		fprintf(fp, "\n");
	}
	fclose(fp);
...
{
	int i;
	FILE *fp = fopen("output process.txt", "w");
	for (i = 0; i < N_process; i++)
	{
		fprintf(fp, "node: %d\n", i);
		fprintf(fp, "    removable: %d\n", process[i].removable);
		fprintf(fp, "    removed: %d\n", process[i].removed);
		fprintf(fp, "    logic gate: %d\n", process[i].logic_gate);
		if (process[i].logic_gate == 3)
		{
			fprintf(fp, "    least event: %d\n", process[i].least_event);
		}
	}
	fclose(fp);
}
...
//输出功能结点
void fprint_function()
{
	int i;
	Linked_List *p;
	FILE *fp = fopen("output function.txt", "w");
	for (i = 0; i < N_function; i++)
	{
		fprintf(fp, "node: %d\n", i);
		fprintf(fp, "    machine: ");
		p = function[i].machine;
		while (p != NULL)
		{
...
//输出标准正态分布表
void fprint_standard_normal_distribution_table()
{
	FILE *fp = fopen("output standard normal distribution table.txt", "w");
	int row_sum = MAXN_row;
	int column_sum = MAXN_column;
	for (int i = 0; i < row_sum; i++)
	{
		...
		{
			fprintf(fp, "%10.6lf", standard_normal_distribution_table[i][j]);
		}
		fprintf(fp, "\n");
	}
	fclose(fp);
...
void fprint_machine()
{
	int i, j;
	Linked_List *p;
	FILE *fp = fopen("output machine.txt", "w");
	for (i = 0; i < N_machine; i++)
	{
		fprintf(fp, "node: %d\n", i);
		fprintf(fp, "    initial capacity: %lf\n", machine[i].initial_capacity);
		fprintf(fp, "    backup initial sum: ");
		for (j = 0; j < backup_place_sum; j++)
		{
			fprintf(fp, "%d ", machine[i].backup_initial_sum[j]);
		}
		fprintf(fp, "\n");
		fprintf(fp, "    backup delay: %d\n", machine[i].backup_delay);
		fprintf(fp, "    backup capacity ratio: %lf\n", machine[i].backup_capacity_ratio);
		fprintf(fp, "    repair reliability: %lf\n", machine[i].repair_reliability);
		fprintf(fp, "    repair unrecoverable rate: %lf\n", machine[i].repair_unrecoverable_rate);
		fprintf(fp, "    component: ");
		p = machine[i].component;
		while (p != NULL)
		{
			fprintf(fp, "%d(%d) ", p->node_num, p->node_sum);
			p = p->next;
		}
		fprintf(fp, "\n");
	}
...
//输出可靠度
void fprint_reliability()
{
	int i, j;
	FILE *fp = fopen("output reliability.txt", "w");
	fprintf(fp, "          ");
	for (i = 0; i < N_machine; i++)
	{
		fprintf(fp, "%10d", i);
	...
	for (i = 0; i < time_sum; i++)
	{
		...
		for (j = 0; j < N_machine; j++)
		{
			fprintf(fp, "%10.6lf", machine[j].reliability[i]);
		}
		fprintf(fp, "\n");
	}
	fclose(fp);
...
//输出零件结点
void fprint_component()
{
	int i, j;
	Linked_List *p;
	FILE *fp = fopen("output component.txt", "w");
	for (i = 0; i < N_component; i++)
	{
		fprintf(fp, "node: %d\n", i);
		fprintf(fp, "    backup initial sum: ");
		for (j = 0; j < backup_place_sum; j++)
		{
			fprintf(fp, "%d ", component[i].backup_initial_sum[j]);
		}
		fprintf(fp, "\n");
		fprintf(fp, "    backup minimum sum: ");
		for (j = 0; j < backup_place_sum - 1; j++)
		{
			fprintf(fp, "%d ", component[i].backup_minimum_sum[j]);
		}
		fprintf(fp, "\n");
		fprintf(fp, "    repair time: %d\n", component[i].repair_time);
		fprintf(fp, "    repair place: %d\n", component[i].repair_place);
		fprintf(fp, "    resource: ");
		p = component[i].resource;
		while (p != NULL)
		{
			fprintf(fp, "%d(%d) ", p->node_num, p->node_sum);
			p = p->next;
		...
		fprintf(fp, "    storage_cost_per_volume: ");
		for (j = 0; j < backup_place_sum; j++)
		{
			fprintf(fp, "%lf ", component[i].storage_cost_per_volume[j]);
		}
		fprintf(fp, "\n");
	}
	fclose(fp);
...
//输出资源结点
void fprint_resource()
{
	int i, j;
	FILE *fp = fopen("output resource.txt", "w");
	for (i = 0; i < N_resource; i++)
	{
		fprintf(fp, "node: %d\n", i);
		fprintf(fp, "    backup initial sum: ");
		for (j = 0; j < repair_place_sum; j++)
		{
			fprintf(fp, "%d ", resource[i].backup_initial_sum[j]);
		}
		fprintf(fp, "\n");
		fprintf(fp, "    repair cost per time unit: %lf\n", resource[i].repair_cost_per_time_unit);
	}
	fclose(fp);
}
...
//输出交通参数
void fprint_transport()
{
	int i, j;
	FILE *fp1 = fopen("output transport time.txt", "w");
	FILE *fp2 = fopen("output transport cost.txt", "w");
	fprintf(fp1, "      ");
	for (i = 0; i < backup_place_sum; i++)
	{
		fprintf(fp1, "%5d", i);
	}
	fprintf(fp1, "\n");
	for (i = 0; i < backup_place_sum; i++)
	{
		...
		for (j = 0; j < backup_place_sum; j++)
		{
			fprintf(fp1, "%5d", transport.time[i][j]);
		}
		fprintf(fp1, "\n");
	}
	fprintf(fp2, "%lf", transport.cost);
	fclose(fp1);
	fclose(fp2);
}
...
//输出遍历过程结点的顺序
void fprint_sequence_process()
{
	int i, j;
	FILE *fp = fopen("output sequence process.txt", "w");
	for (i = 0; i < N_process_exist; i++)
	{
		fprintf(fp, "%d ", sequence_process[i]);
	}
	fclose(fp);
}
...
int main()
{
	...
	fprint_adjacency_matrix();
	fprint_process();
	fprint_function();
	fprint_standard_normal_distribution_table();
	fprint_reliability();
	fprint_machine();
	fprint_component();
	fprint_resource();
	fprint_transport();
	fprint_sequence_process();
	srand((unsigned)time(NULL));
	for (int i = 0; i < test_sum; i++)
	{
		printf("test num: %d\n", i + 1);
		fprintf(fp_test_result, "test num: %d\n", i + 1);
		refresh_parameter();
		test();
	}
	fclose(fp_test_result);
	fprint_result();
    return 0;
}
